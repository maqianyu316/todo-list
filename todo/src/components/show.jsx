 ...
 * @LastEditTime: 2019-11-29 16:25:46
 * @LastEditors: xiaoma
 */
  ...
  render() {
    ...
    return (
      <div className="list">
        <ul className="list-group">
          {data.map((item, index) => (
            <li
              style={{
                textDecoration:
                  item.status === "finished" ? "line-through" : "none",
                color:
              ...
              <div>
                {item.status !== "finished" && (
                  <i
                    ...
                    onClick={() => this.finishAction(index)}
                  ></i>
                )}
                {item.status === "finished" && (
                  <i
                    className="glyphicon glyphicon-share-alt"
                    onClick={() => this.withdraw(index)}
                  ></i>
                )}
              </div>
              <div>
        ...
        <div>
          {data.length !== 0 && all !== finished ? (
            <i
              className="glyphicon glyphicon-check"
              onClick={() => this.fininshAll()}
            ></i>
          ) : null}
          {data.length !== 0 && all === finished ? (
            <i
